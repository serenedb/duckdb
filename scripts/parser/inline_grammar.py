...
# (category key, constexpr array name, membership-set name)
_KEYWORD_SET_LAYOUT = [
    ("RESERVED_KEYWORD", "kReservedKeywords", "kReservedSet"),
    ("UNRESERVED_KEYWORD", "kUnreservedKeywords", "kUnreservedSet"),
    ("COL_NAME_KEYWORD", "kColNameKeywords", "kColNameSet"),
    ("TYPE_FUNC_NAME_KEYWORD", "kTypeFuncKeywords", "kTypeFuncSet"),
    ("TYPE_NAME_KEYWORD", "kTypeNameKeywords", "kTypeNameSet"),
]


def _write_keyword_array(f, name, keywords):
    # One keyword per line -- keeps git diffs to exactly the keywords that changed.
    f.write(f"constexpr std::string_view {name}[] = {{\n")
...
def write_keyword_map(keyword_sets):
    # The keyword categories are case-insensitive membership sets: one constexpr
    # std::string_view array + one flat_hash_set<string_view> (case_insensitive_set_view_t)
    # per category, built once at static init from the array. Lookups
    # (KeywordCategoryType/IsKeyword/KeywordList) are defined here so they see the
    # file-local sets. Keyword content is lowercase (see load_keywords).
    ...
    with open(output_path, "w") as f:
        ...
        f.write('#include "duckdb/parser/simplified_token.hpp"\n\n')
        f.write('#include "duckdb/common/case_insensitive_map.hpp"\n\n')
        f.write("#include <iterator>\n")
        f.write("#include <string_view>\n\n")
        f.write("namespace duckdb {\n")
        f.write("namespace {\n\n")
        for category, array_name, _set_name in _KEYWORD_SET_LAYOUT:
            _write_keyword_array(f, array_name, keyword_sets[category])
        f.write("\n")
        for _category, array_name, set_name in _KEYWORD_SET_LAYOUT:
            f.write(
                f"const case_insensitive_set_view_t {set_name}(std::begin({array_name}), std::end({array_name}));\n"
            )
        f.write("\n}  // namespace\n\n")

        f.write("bool peg::KeywordCategoryType(std::string_view text, PEGKeywordCategory type) {\n")
        f.write("\tswitch (type) {\n")
        for enum_name, set_name in [
            ("KEYWORD_RESERVED", "kReservedSet"),
            ("KEYWORD_UNRESERVED", "kUnreservedSet"),
            ("KEYWORD_TYPE_FUNC", "kTypeFuncSet"),
            ("KEYWORD_COL_NAME", "kColNameSet"),
            ("KEYWORD_TYPE_NAME", "kTypeNameSet"),
        ]:
            f.write(f"\tcase PEGKeywordCategory::{enum_name}:\n\t\treturn {set_name}.contains(text);\n")
        f.write("\tdefault:\n\t\treturn false;\n\t}\n}\n\n")

        # IsKeyword/KeywordList intentionally exclude the TypeName-only set (type
        # names stay identifier-usable; no public KeywordCategory::TYPE_NAME).
        f.write("bool peg::IsKeyword(std::string_view text) {\n")
        f.write("\treturn kReservedSet.contains(text) || kUnreservedSet.contains(text) ||\n")
        f.write("\t       kColNameSet.contains(text) || kTypeFuncSet.contains(text);\n}\n\n")

        f.write("vector<ParserKeyword> peg::KeywordList() {\n")
        f.write("\tvector<ParserKeyword> result;\n")
        for array_name, enum_name in [
            ("kReservedKeywords", "KEYWORD_RESERVED"),
            ("kUnreservedKeywords", "KEYWORD_UNRESERVED"),
            ("kTypeFuncKeywords", "KEYWORD_TYPE_FUNC"),
            ("kColNameKeywords", "KEYWORD_COL_NAME"),
        ]:
            f.write(f"\tfor (auto kw : {array_name}) {{\n")
            f.write(f"\t\tresult.push_back({{string(kw), KeywordCategory::{enum_name}}});\n\t}}\n")
