...
static int FigureColnameInternal(const BaseExpression &expr, string &name) {
	...
	case ExpressionClass::COLUMN_REF: {
		...
		name = column_ref.GetColumnName().GetIdentifierName();
		return 2;
	}
	case ExpressionClass::FUNCTION: {
		auto &function = expr.Cast<FunctionExpression>();
		if (function.IsOperator() || function.IsFromOperator()) {
			// PG: operators (arithmetic, LIKE, and regex ~/SIMILAR TO desugared to functions)
			// don't produce column names
			return 0;
		}
