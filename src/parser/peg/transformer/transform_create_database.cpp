...
// DROP DATABASE foo [(FORCE)] -> DETACH foo with is_drop=true so duckdb routes through
// DatabaseManager::DropDatabase (PG-compatible "database X does not exist" error and storage-extension drop_database
// callback). FORCE is parsed for grammar compatibility but currently ignored (serenedb's drop is already synchronous).
