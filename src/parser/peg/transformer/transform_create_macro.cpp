...
unique_ptr<MacroFunction> PEGTransformerFactory::TransformMacroDefinition(PEGTransformer &transformer,
	...
	if (decorator_info.has_language && macro_function->type == MacroType::SCALAR_MACRO) {
		...
		if (scalar.expression && scalar.expression->GetExpressionClass() == ExpressionClass::CONSTANT) {
			...
			if (val.type().id() == LogicalTypeId::VARCHAR && !val.IsNull()) {
				...
				// PG-compat (port of v2026.05.18 TransformMacroFunction): the SQL
				// body may be SELECT (read-only) or DML (INSERT/UPDATE/DELETE).
				// All of these store a QueryNode subclass that TableMacroFunction
				// can wrap, so procedures with DML bodies execute the statement
				// on CALL instead of returning the body text as a scalar string.
	...
	// PG-compat (port of the "Apply RETURNS TABLE column aliases" block from
	// v2026.05.18's TransformMacroFunction): when the user declared
	// `RETURNS TABLE(col type, ...)` and the body is a SELECT, propagate the
	// declared column names onto the body. For `SELECT *`-shaped bodies
	// (e.g. a VALUES table), push the names onto the FROM table's
	// column_name_alias (and ExpressionListRef::expected_names for VALUES).
	// For per-expression bodies, alias each select_list entry.
