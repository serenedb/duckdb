...
unique_ptr<SQLStatement> PEGTransformerFactory::TransformCreatePublicationStatement(PEGTransformer &transformer,
	...
	result->info->parameters.push_back(make_uniq<ConstantExpression>(Value(pub_name.GetIdentifierName())));
		...
		if (for_choice.name == "PublicationForAllTables") {
			...
			for (idx_t i = 0; i < table_results.size(); i++) {
				...
				if (!qualified_name.Schema().empty()) {
					tables_value +=
					    qualified_name.Schema().GetIdentifierName() + "." + qualified_name.Name().GetIdentifierName();
				} else {
					tables_value += qualified_name.Name().GetIdentifierName();
				}
			}
