...
void DatabaseManager::DropDatabase(ClientContext &context, const Identifier &name, OnEntryNotFound if_not_found) {
	...
	if (ext && ext->drop_database) {
		...
		ext->drop_database(context, name.GetIdentifierName(), if_not_found);
		DetachInternal(name);
	} else if (found_db) {
		throw BinderException("Database \"%s\" does not support DROP DATABASE", name.GetIdentifierName());
	} else if (if_not_found == OnEntryNotFound::THROW_EXCEPTION) {
		throw BinderException("database \"%s\" does not exist", name.GetIdentifierName());
	}
}
...
void DatabaseManager::RenameDatabase(ClientContext &context, const Identifier &old_name, const Identifier &new_name,
	...
	if (GetDefaultDatabase(context) == old_name) {
