...
CatalogEntry &LogicalCreateIndex::BindTable(ClientContext &context, CreateIndexInfo &info_p) {
	...
	auto entry = Catalog::GetEntry<TableCatalogEntry>(context, QualifiedName(catalog, schema, table_name),
	                                                  OnEntryNotFound::RETURN_NULL);
	if (entry) {
		return *entry;
	}
	return Catalog::GetEntry<ViewCatalogEntry>(context, QualifiedName(catalog, schema, table_name));
}
