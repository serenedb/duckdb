...
BoundStatement Binder::Bind(TableFunctionRef &ref) {
	...
	if (need_scalar_wrap) {
		if (!func_catalog_ptr) {
			EntryLookupInfo scalar_function_lookup(CatalogType::SCALAR_FUNCTION_ENTRY,
			                                       QualifiedName(fexpr.FunctionName()), error_context);
			func_catalog_ptr = GetCatalogEntry(catalog, schema, scalar_function_lookup, OnEntryNotFound::RETURN_NULL);
			if (!func_catalog_ptr) {
				EntryLookupInfo scalar_macro_lookup(CatalogType::MACRO_ENTRY, QualifiedName(fexpr.FunctionName()),
				                                    error_context);
		...
		if (!func_catalog_ptr) {
			func_catalog_ptr =
			    GetCatalogEntry(catalog, schema, table_function_lookup, OnEntryNotFound::THROW_EXCEPTION);
		}
	}
