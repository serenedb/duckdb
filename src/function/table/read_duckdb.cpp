...
void DuckDBLookupScan(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
	...
	// LookupScan wrote each output row's requested-pk index into pk_survivors; remap to the caller-side
	// gather index (pk_output_positions) and publish the compacted cardinality. With no pushed filters the
	// count equals pk_lookups (misses kept as NULL rows); with filters it is the survivor count.
	for (idx_t k = 0; k < w; ++k) {
		data.pk_survivors[k] = data.pk_output_positions[data.pk_survivors[k]];
	}
