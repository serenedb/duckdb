...
public:
	DUCKDB_API FunctionExpression(string catalog_name, string schema_name, std::string_view function_name,
	...
	DUCKDB_API FunctionExpression(std::string_view function_name, vector<unique_ptr<ParsedExpression>> children,
	                              unique_ptr<ParsedExpression> filter = nullptr,
	                              unique_ptr<OrderModifier> order_bys = nullptr, bool distinct = false,
	                              bool is_operator = false, bool export_state = false);
