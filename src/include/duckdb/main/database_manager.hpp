...
public:
	...
	//! Records that every id up to its argument is spent, returning whether the record was made durable.
	//! Set by a host that persists the allocator; without one the ids are process-local and nothing is
	//! written down. A false return leaves the horizon unraised, so the next allocation asks again --
	//! raising it on a failed write would hand out ids a crash could reissue.
	void SetOidReservationSink(bool (*sink)(idx_t)) {
...
private:
	...
	atomic<bool (*)(idx_t)> oid_reservation_sink;
