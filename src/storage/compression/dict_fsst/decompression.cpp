...
string_t CompressedStringScanState::ReconstructEntry(ArenaAllocator &allocator, uint32_t pid, uint32_t len,
	...
	if (pid >= prefix_count) {
		...
		if (!decoder) {
			// DICTIONARY: the bytes are stored raw, not FSST-encoded.
...
void CompressedStringScanState::Initialize(bool initialize_dictionary) {
	...
	if (!IsNativeMode(mode) && !IsPlusMode(mode)) {
		// Whitelist, not `< COUNT`: values 3-13 are reserved for future vanilla-duckdb modes, so a block carrying one
		// must be rejected here rather than misread as a native or FSST+ segment.
	...
	// Resolve the block layout for this mode: the FSST symbol table, the row->entry selection buffer, the encoded
	// entry byte source (dict_ptr), and the bitpacked source of the per-entry lengths (entry_lengths_src). Native
	// modes store whole strings; plus modes cleave each entry into a shared prefix + suffix, so dict_ptr is the
	// suffix region and they additionally carry a prefix pool. Both keep dict_count per-entry lengths with index 0
	// the null slot, so the entry index is just the selection value and everything below is mode-agnostic.
	...
	// FSST symbol table: present for every mode except the plain (mode 0) DICTIONARY layout.
	...
	// Only the deduplicated modes emit a DictionaryVector; FSST_ONLY / FSST_PLUS are row-order (no selection) and
	// the fetch path never emits one. Modes that don't materialize just unpack the per-entry metadata their
	// on-demand reconstruct (FetchEntry) needs, then return.
	const bool materialize =
	    initialize_dictionary && mode != DictFSSTMode::FSST_ONLY && mode != DictFSSTMode::FSST_PLUS;

	// Whole segment fits inline (<= 12 bytes/value)? Then every decode below (materialize and on-demand fetch) can
	// go straight into an inline string_t and skip the arena. Computed here so it also covers the materialize path;
	// it previously lived in StringInitScan, which runs after Initialize and so never reached the materialize loop.
	...
	// Decode the shared prefixes once (none for native, where prefix_count == 0) into one contiguous uninitialized
	// buffer, tracking each prefix's offset/length. A per-entry reconstruct is then a memcpy + one suffix decode.
	// The encoded prefix lengths are read sequentially, so stream them through a 32-value stack group (no alloc).
	if (prefix_count > 0) {
	...
	if (!materialize) {
		// Fetch / FSST_(PLUS): reconstruct at random entry indices, so unpack the full per-entry length (+ plus
		// prefix-id) arrays. Byte offsets come from DecompressOffset (a lazy prefix sum of entry_lengths).
	...
	// One materialize loop for every mode: stream entry_lengths (and, for plus, prefix_ids) through 32-value stack
	// groups, walk dict_ptr with a running offset, and reconstruct each entry directly into its dict slot -- no
	// full-length arrays on this thread, which is what stopped the plus scan thrashing the L2 shared between two SMT
	// siblings. Entry 0 is the null slot (length 0 -> empty; the validity mask already marked the slot NULL).
...
void CompressedStringScanState::ScanToFlatVector(Vector &result, idx_t result_offset, idx_t start, idx_t scan_count) {
	...
	if (mode == DictFSSTMode::FSST_ONLY || mode == DictFSSTMode::FSST_PLUS) {
		// Row-order (no selection, no nulls): entry index IS the row, +1 past the null slot native keeps at index 0.
		auto &allocator = StringVector::GetStringAllocator(result);
		const idx_t base = start + 1; // +1 past the null slot at index 0
		for (idx_t i = 0; i < scan_count; i++) {
			result_data.WriteStringRef(FetchEntry(allocator, base + i));
		}
		result.Verify();
		return;
	}
