...
inline constexpr idx_t DUCKDB_VERSION_NUMBER_DEFAULT = DuckDBVersionNumber(DUCKDB_VERSION_DEFAULT);
inline constexpr idx_t DUCKDB_VERSION_NUMBER_LOWER = DuckDBVersionNumber(DUCKDB_VERSION_LOWER);
inline constexpr idx_t DUCKDB_VERSION_NUMBER_UPPER = DuckDBVersionNumber(DUCKDB_VERSION_UPPER);
inline constexpr idx_t SERENEDB_VERSION_NUMBER_LOWER = SereneDBVersionNumber(SERENEDB_VERSION_LOWER);
inline constexpr idx_t SERENEDB_VERSION_NUMBER_UPPER = SereneDBVersionNumber(SERENEDB_VERSION_UPPER);
...
bool IsReadableStorageVersion(const StorageVersion version) {
	...
	if (duckdb_version < DUCKDB_VERSION_NUMBER_LOWER || DUCKDB_VERSION_NUMBER_UPPER < duckdb_version) {
		return false;
	}
	const auto serenedb_version = SereneDBVersionNumber(version);
	if (serenedb_version == 0) {
		return true;
	}
	if (serenedb_version < SERENEDB_VERSION_NUMBER_LOWER || SERENEDB_VERSION_NUMBER_UPPER < serenedb_version) {
		return false;
	}
	return true;
}
...
string StorageVersionInfo::GetStorageVersionString(const StorageVersion &version) {
	...
	throw InvalidInputException("Invalid storage version %llu", version);
}
