...
const uint64_t DUCKDB_VERSION_NUMBER_DEFAULT = DuckdbVersionNumber(StorageVersionInfo::DEFAULT_STORAGE_VERSION_INFO);
const uint64_t DUCKDB_VERSION_NUMBER_LOWER = DuckdbVersionNumber(StorageVersion::V0_9_0);
const uint64_t DUCKDB_VERSION_NUMBER_UPPER = DuckdbVersionNumber(StorageVersion::DUCKDB_LATEST);
const uint64_t SERENEDB_VERSION_NUMBER_UPPER = SerenedbVersionNumber(StorageVersion::SERENEDB_LATEST);

static_assert(DUCKDB_VERSION_NUMBER_LOWER <= DUCKDB_VERSION_NUMBER_DEFAULT, "Check on VERSION_NUMBER lower bound");
static_assert(DUCKDB_VERSION_NUMBER_DEFAULT <= DUCKDB_VERSION_NUMBER_UPPER, "Check on VERSION_NUMBER upper bound");
// A serenedb version is "this duckdb base plus our patches", so it must be built on the newest duckdb
// version this tree has. A rebase that bumps duckdb without adding the matching serenedb version fails
// here rather than silently claiming features of a base we were never built on.
static_assert(DuckdbVersionNumber(StorageVersion::SERENEDB_LATEST) == DUCKDB_VERSION_NUMBER_UPPER,
              "SereneDB's newest version must be built on duckdb's newest version");

bool IsReadableStorageVersion(const StorageVersion version) {
	const auto duckdb_version = DuckdbVersionNumber(version);
	if (duckdb_version < DUCKDB_VERSION_NUMBER_LOWER || duckdb_version > DUCKDB_VERSION_NUMBER_UPPER) {
		return false;
	}
	return SerenedbVersionNumber(version) <= SERENEDB_VERSION_NUMBER_UPPER;
}
...
string GetStorageVersionNameInternal(const StorageVersion storage_version) {
	...
	for (idx_t i = 0; storage_version_info[i].version_name; i++) {
		if (strcmp(storage_version_info[i].version_name, "latest") == 0 ||
		    strcmp(storage_version_info[i].version_name, "serenedb_latest") == 0) {
			continue;
		}
...
string StorageVersionInfo::GetStorageVersionString(const StorageVersion &version) {
	...
	throw InvalidInputException("Invalid storage version %llu", static_cast<uint64_t>(version));
}
