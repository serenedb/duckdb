...
public:
	unique_ptr<StorageLockKey> GetExclusiveLock() ABSL_NO_THREAD_SAFETY_ANALYSIS {
		exclusive_lock.lock();
		while (read_count != 0) {
		}
	...
	unique_ptr<StorageLockKey> TryGetExclusiveLock() ABSL_NO_THREAD_SAFETY_ANALYSIS {
	...
	unique_ptr<StorageLockKey> TryUpgradeCheckpointLock(StorageLockKey &lock) ABSL_NO_THREAD_SAFETY_ANALYSIS {
	...
	void ReleaseExclusiveLock() ABSL_NO_THREAD_SAFETY_ANALYSIS {
		exclusive_lock.unlock();
	}
