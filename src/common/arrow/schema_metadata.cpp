...
ArrowSchemaMetadata::ArrowSchemaMetadata(const char *metadata) {
	...
	extension_metadata_map = StringUtil::ParseJSONMap(schema_metadata_map[ARROW_METADATA_KEY], ignore_errors)->Flatten();
}
...
static string GetMapValue(const unordered_map<string, string> &map, const string &key) {
	auto it = map.find(key);
	return it == map.end() ? string() : it->second;
}
