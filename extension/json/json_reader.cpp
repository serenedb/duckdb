...
void JSONReader::AddVirtualColumn(column_t virtual_column_id) {
	...
	// AddVirtualColumn, so column_ids.size() - 1 is the output slot this virtual column occupies.
	// ReadJSONFunction later writes byte offsets into output.data[file_row_number_local_idx].
	file_row_number_local_idx = column_ids.size() - 1;
}
