...
struct ICUBucket : public ICUDateFunc {
	...
	static timestamp_tz_t UnbucketOf(const BindData &info, const BucketSpec &spec, int64_t bucket,
		...
		int64_t value = 0;
		if (!TryMultiplyOperator::Operation(bucket, spec.width, value) ||
		    !TryAddOperator::Operation(value, spec.anchor, value)) {
			throw OutOfRangeException("Time zone bucket %lld of width %lld is out of range", bucket, spec.width);
		}
		int64_t day = 0;
		switch (spec.kind) {
		case BucketSpec::Kind::INSTANT:
	...
	static unique_ptr<BucketRewrite> NameRewrite(const BoundFunctionExpression &expr, std::string_view part,
