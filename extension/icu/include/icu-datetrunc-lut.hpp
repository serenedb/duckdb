...
struct ICUDateTruncLUT {
	...
	static bool TryGetConstantPart(Vector &part_arg, DatePartSpecifier &part) {
		if (part_arg.GetVectorType() != VectorType::CONSTANT_VECTOR || ConstantVector::IsNull(part_arg)) {
			return false;
		}
		return TryGetDatePartSpecifier(ConstantVector::GetData<string_t>(part_arg)->GetString(), part);
	}
	...
	static bool TryExecute(DataChunk &args, ExpressionState &state, Vector &result) {
		...
		if (!TryGetConstantPart(args.data[0], part) || part == DatePartSpecifier::ERA) {
			return false;
		}
		...
		DateTrunc::Dispatch(part, [&](auto op) {
			UnaryExecutor::Execute<timestamp_tz_t, timestamp_tz_t>(
			    args.data[1], result, args.size(), [&](timestamp_tz_t input) {
				    ...
				    return Truncate(info, calendar, part, input);
			    });
		});
		return true;
	}
