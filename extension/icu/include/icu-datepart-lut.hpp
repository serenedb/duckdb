...
struct ICUDatePartLUT {
	struct LocalTime {
		timestamp_t wall = timestamp_t(0);
		int64_t offset = 0;
		int32_t days = 0;
	...
	template <class RESULT, class FALLBACK>
	...
	template <class RESULT, class FALLBACK>
	static bool TryUnary(DataChunk &args, ExpressionState &state, Vector &result, FALLBACK &&outside) {
		...
		return TryExecute<RESULT>(info.lut.get(), args.data[0], result, args.size(),
		                          ...
		                          std::forward<FALLBACK>(outside));
	}

	template <class FALLBACK>
	static bool TryBinary(DataChunk &args, ExpressionState &state, Vector &result, FALLBACK &&outside) {
		DatePartSpecifier part;
		if (!ICUConstantArgs::TryGetPart(args.data[0], part)) {
			return false;
		}
		auto &expr = state.expr.Cast<BoundFunctionExpression>();
		auto &info = expr.BindInfo()->Cast<ICUDateFunc::BindData>();
